...
export const load: PageServerLoad = async ({ params, parent }) => {
  // get authentication
  const { loggedIn, userID: currentUser } = await parent();
...
export const actions: Actions = {
  tag: async ({ params, cookies }) => {
    ...
    const currentUserGameIDs = currentUser.games.map(({ gameID }) => gameID);

    const targetGameDataIDs = targetUser.games
      .filter(({ gameID }) => currentUserGameIDs.includes(gameID))
      .map(({ id }) => id);
    if (targetGameDataIDs.length === 0)
      return fail(400, {
        message: `You and ${
          targetUser.nickname ?? "@" + targetUser.id
    ...
    await prisma.tag.create({
      data: {
        ...
        remainingGameData: {
          connect: targetGameDataIDs.map((id) => ({ id })),
        },
        gameDataIDs: targetGameDataIDs,
      },
    });
